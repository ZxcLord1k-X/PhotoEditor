...
def showFileNamesList():
    extentions = [".jpg", ".jpeg", ".png", ".svg", ".gif", ".bmp"]
    chooseWorkdir()
    filenames = filter(os.listdir(work_dir), extentions)
    lw_files.clear()
...
def showChosenImage():  # Оголошення функції `showChosenImage`, яка відображає вибране зображення з переліку файлів.
    if lw_files.currentRow() >= 0:  # Перевіряє, чи обраний будь-який елемент у списку `lw_files` (чи є активний рядок).
        ...
        workimage.loadImage(work_dir, filename)  # Завантажує вибране зображення, передаючи робочу директорію та ім'я файлу.
